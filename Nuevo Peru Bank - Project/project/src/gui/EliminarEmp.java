...
public class EliminarEmp extends javax.swing.JFrame {
    ...
    public EliminarEmp(String idEmp) {
        this.idEmpl=idEmp;
        this.setTitle("Nuevo Perú Bank - Eliminar Empleado");
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Digite el DNI del Empleado a eliminar.");
        jPanel1.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(250, 150, -1, 30));
        ...
        jPanel1.add(txtDni, new org.netbeans.lib.awtextra.AbsoluteConstraints(230, 200, 380, 50));
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        String dni=txtDni.getText(); // Obtenemos el dni introducido y lo almacenamos, lo usaremos como condicion
        try (Connection conn = ConexionOracle.getConnection()){
            String sql="DELETE FROM empleado WHERE dni = ?"; // ejecutamos la sentencia sql para borrar al empleado dependiendo de el dni ingresado
            PreparedStatement ps=conn.prepareStatement(sql); // preparamos la sentencia
            ps.setString(1, dni); // ingresamos la informacion al campo dni mediante el index en el que se encuentra y el valor que almacenamos anteriormente
            int filas=ps.executeUpdate(); // metodo para hacer actualizaciones y eliminaciones 
            if (filas > 0) { // si en la consulta anterior se eliminaron filas lanzamos un mensaje de confirmacion
                JOptionPane.showMessageDialog(null, "Empleado eliminado correctamente.", "Nuevo Perú Bank", JOptionPane.INFORMATION_MESSAGE);
        } else { // si no se eliminaron filas con esa condicional es porque ese dni no estaba vinculado a ningun empleado
            JOptionPane.showMessageDialog(null, "No se encontró un empleado con ese DNI.", "Nuevo Perú Bank - Error", JOptionPane.ERROR_MESSAGE);
        }
